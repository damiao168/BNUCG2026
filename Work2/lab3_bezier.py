...
@ti.kernel
def clear_pixels(pixels: ti.template()):
    for i, j in pixels:
        pixels[i, j] = ti.Vector([0.0, 0.0, 0.0])

@ti.kernel
def draw_curve_kernel(n: ti.i32, pixels: ti.template(), curve_points_field: ti.template(), WIDTH: ti.i32, HEIGHT: ti.i32):
    for i in range(n):
...
def main():
    ...
    while window.running:
        ...
        if current_count >= 2:
            ...
            draw_curve_kernel(NUM_SEGMENTS + 1, pixels, curve_points_field, WIDTH, HEIGHT)
